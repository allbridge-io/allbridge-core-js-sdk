...
export class BitVector implements StructClass {
  ...
  static get $typeName() {
    return `${PKG_V13}::bit_vector::BitVector`;
  }
  ...
  private constructor(typeArgs: [], fields: BitVectorFields) {
    this.$fullTypeName = composeSuiType(BitVector.$typeName, ...typeArgs) as string;
  ...
  static reified(): BitVectorReified {
    return {
      ...
      fullTypeName: composeSuiType(BitVector.$typeName, ...[]) as string,
      typeArgs: [] as [],
