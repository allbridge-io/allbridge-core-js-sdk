...
export class ObjectBag implements StructClass {
  ...
  static get $typeName() {
    return `${PKG_V26}::object_bag::ObjectBag`;
  }
  ...
  private constructor(typeArgs: [], fields: ObjectBagFields) {
    this.$fullTypeName = composeSuiType(ObjectBag.$typeName, ...typeArgs) as string;
  ...
  static reified(): ObjectBagReified {
    return {
      ...
      fullTypeName: composeSuiType(ObjectBag.$typeName, ...[]) as string,
      typeArgs: [] as [],
