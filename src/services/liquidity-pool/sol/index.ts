...
export class SolanaPool extends Pool {
  ...
  constructor(
    public params: SolanaPoolParams,
  ...
  async getUserBalanceInfo(
    ...
    try {
      ...
      const { lpAmount, rewardDebt } = await bridge.account.userDeposit.fetch(
        userDepositAccount
      );
  ...
  async getPoolInfo(token: TokenInfoWithChainDetails): Promise<PoolInfo> {
    ...
    const pool = await this.getBridge(
      token.bridgeAddress,
      provider
    ).account.pool.fetch(token.poolAddress);
    return {
  ...
  async buildRawTransactionDeposit(
    ...
    const { bridge, accounts, preInstructions } =
      await this.prepareDataForTransaction(params);
  ...
  private async _getLPTransactionData(
    ...
    try {
      ...
      const createUserTokenInstruction: TransactionInstruction =
        await associatedProgram.methods
          .create()
          .accounts({
